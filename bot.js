...
function iniciarVotacion(ctx) {
    ...
    ctx.reply('🏁 Ronda terminada. Voten en privado.');
    
    // Temporizador para finalizar ronda automáticamente después de votar
    setTimeout(() => {
        rondaActual++;
        if (rondaActual <= 5) {
            ctx.reply(`🔄 Ronda ${rondaActual} comenzando...`);
            iniciarRonda(ctx);
        } else {
            anunciarGanador(ctx);
        }
...
bot.action(/voto_(.+)/, (ctx) => {
    votos.set(ctx.from.id, ctx.match[1]);
    ctx.answerCbQuery('✅ Voto registrado');
    ctx.editMessageText('✅ Voto registrado.');
});
